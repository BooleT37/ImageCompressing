...
class ImageCompressor:
    @staticmethod
    def uniformQuantizeRgb(pixels, depth=2):
        if depth > 8:
            raise ImageCompressorException("Depth can't be more than 8!")
        divisor = 8 - depth
        newPixels = list(map(lambda pixel: (
            (pixel[0] >> divisor << divisor) + 2 ** (divisor - 1),
            (pixel[1] >> divisor << divisor) + 2 ** (divisor - 1),
            (pixel[2] >> divisor << divisor) + 2 ** (divisor - 1)), pixels))
        # print (pixels[0], newPixels[0])
        return newPixels

    @staticmethod
    def uniformQuantizeYCbCr(pixels, method):
        ...
        newYCbCrPixels = list(map(lambda pixel: ((pixel[0] >> divisors[0] << divisors[0]) + 2 ** (divisors[0] - 1),
                                                 (pixel[1] >> divisors[1] << divisors[1]) + 2 ** (divisors[1] - 1),
                                                 (pixel[2] >> divisors[2] << divisors[2]) + 2 ** (divisors[2] - 1)),
                                  YCbCrPixels))
