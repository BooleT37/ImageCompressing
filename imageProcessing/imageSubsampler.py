	...
	def SubsampleImage(self, rgbPixels, mode, shrinkImage = False):
		...
		for i in range(0, height, 2):
			...
			for j in range(0, width, 2):
				lastCol = (j == width - 1)
				pixQuad = [pixels[i][j], pixels[i][j + 1], pixels[i + 1][j], pixels[i + 1][j + 1]]
				newPixQuad = [(pixel[k][0], pixel[pixelTransitions[mode][k]][1], pixel[pixelTransitions[mode][k]][2]) for k, pixel in enumerate(pixQuad)]
