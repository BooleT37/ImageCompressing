...
def createHistogram(pixels):
    return [createHistogramForOneColor(pixels, 0), createHistogramForOneColor(pixels, 1),
            createHistogramForOneColor(pixels, 2)]
...
class PartitionElement:
    ...
    def splitInTwo(self, median, colorIndex):
        ...
        el1Histogram = [Histogram(self.histogram[0][:]), Histogram(self.histogram[1][:]),
                        Histogram(self.histogram[2][:])]
        ...
        el2Histogram = [Histogram(self.histogram[0][:]), Histogram(self.histogram[1][:]),
                        Histogram(self.histogram[2][:])]
...
def createPartitions(pixels):
    ...
    print("Histogram:\n\tRed:\n\n{}\n\tGreen:\n\n{}\n\tBlue:\n\n{}\n".format(histogram[0], histogram[1],
                                                                             histogram[2])) if DEBUG else 0
...
def findPartitionIndex(pixel, partitions):
    for index, prtEl in enumerate(partitions):
        if (prtEl.dot[0] <= pixel[0] <= prtEl.dot[0] + prtEl.widths[0] and
            prtEl.dot[1] <= pixel[1] <= prtEl.dot[1] + prtEl.widths[1] and
            prtEl.dot[2] <= pixel[2] <= prtEl.dot[2] + prtEl.widths[2]
        ):
                return index
        ...
        if index is None:
            ...
            if pixelsQuantized <= FIRST_PIXELS_TO_PRINT:
                ...
                if pixelsQuantized >= firstPixelToPrintInLastGroup:
                    print(
                        "{}. {} -> partition {} -> {}".format(pixelIndex, pixel, index, palette[index])) if DEBUG else 0
        pixelsQuantized += 1
    return newPixels
